...
def fromCMvMonomialEmbedding :
  Function.Embedding (CMvMonomial n) (Fin n →₀ ℕ)
where
  toFun := fromCMvMonomial
  inj' := by
    unfold Function.Injective
    intros m₁ m₂ h
    unfold fromCMvMonomial at h
    simp at h
    apply Finsupp.toFun_eq at h
    split at h; dsimp at h
    apply vector_get_eq
    exact h

-- def fromCMvMonomialSurjective {n} : Function.Surjective (@fromCMvMonomial n) := by
--   sorry
...
#eval toCMvMonomial (fromCMvMonomial myMonomial)
#eval myMonomial
...
theorem monomial_list_nodup (l : List (CMvMonomial n)) :
  l.Pairwise (RBNode.cmpLT compare) → l.Pairwise (· ≠ ·)
:= by
  induction l with
  | nil => simp
  | cons head tail ih =>
    ...
    · intro a a_in
      ...
      have instanceTransMonomial :
        TransCmp (compare : CMvMonomial n → CMvMonomial n → Ordering) := by
          infer_instance
...
lemma pair_list_pairwise_lt (l : List (CMvMonomial n × R)) :
  l.Pairwise (RBNode.cmpLT (compare ·.1 ·.1)) →
    l.unzip.1.Pairwise (RBNode.cmpLT compare)
:= by
  intro h
  induction h with
  ...
  | @cons a l head tail ih =>
    ...
    · intros a' a'_in
      ...
      have instanceTransMonomial :
        TransCmp (λ x1 x2 : (CMvMonomial n × R) => compare x1.1 x2.1) := by
          infer_instance
      simp [RBNode.cmpLT, *]
    · apply ih

lemma list_pairwise_lt_nodup (l : List (CMvMonomial n)) :
  l.Pairwise (RBNode.cmpLT compare) → l.Nodup
:= by
  intro h
  induction h with
  | nil => simp
...
-- def fromMvPolynomial [CommRing R] (p : MvPolynomial σ R) : (CMvPolynomial n R) := sorry
...
:= by
  ...
  case h_2 v h_find =>
    ...
    have ⟨val, find_val⟩ : ∃ r, p.find? m = some r := by
      ...
      have eq_m : compare m m = .eq := by simp
      exists b
...
:= by
  ...
  case h_1 v₀ v h_find =>
    ...
    simp_all only
      [Std.compare_eq_iff_eq, List.unzip_fst, List.mem_map, Prod.exists]
...
:=
  ...
  let mem_support_fun : ∀ (a : Fin n →₀ ℕ), a ∈ support ↔ toFun a ≠ 0 := by
    ...
    · intro a_in
      ...
      obtain ⟨left, right⟩ := h
      subst right
      simp [fromCMvMonomialEmbedding, monomial_id₁]
